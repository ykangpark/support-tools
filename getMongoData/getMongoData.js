 ...
 *
 *     mongo HOST:PORT/admin -u ADMIN_USER -p ADMIN_PASSWORD getMongoData.js >
 * getMongoData.log
 *
 * For details, see
...
function printShardInfo() {
  ...
  printInfo('Balancer status', function() {
    return db.adminCommand({balancerStatus: 1})
  }, section);

  if (sh.getRecentMigrations) {  // Function does not exist in older shell
                                 // versions (2.6 and below)
    printInfo('Recent chunk migrations', function() {
  ...
  if (sh.getRecentFailedRounds) {  // Function does not exist in older shell
                                   // versions (2.6 and below)
    printInfo('Recent failed balancer rounds', function() {
...
function printServerInfo() {
  ...
  printInfo('Host info', function() {
    return db.hostInfo()
  }, section);
  printInfo('Command line info', function() {
    return db.serverCmdLineOpts()
  }, section);
  printInfo('Server build info', function() {
    return db.serverBuildInfo()
  }, section);
  printInfo('Server parameters', function() {
...
function printReplicaSetInfo() {
  ...
  printInfo('Replica set config', function() {
    return rs.conf()
  }, section);
  printInfo('Replica status', function() {
    return rs.status()
  }, section);
  printInfo('Replica info', function() {
    return db.getReplicationInfo()
  }, section);
...
function printUserAuthInfo() {
  ...
  if (typeof db.system.users.countDocuments === 'function') {
    ...
    printInfo('Database user count', function() {
      return db.system.users.count()
    }, section);
    printInfo('Custom role count', function() {
      return db.system.roles.count()
    }, section);
  }
}
...
function collectQueryableEncryptionInfo(isMongoS) {
  ...
  const getAuxiliaryCollectionInfo = function(db, collName) {
    ...
    let isClusteredCollection = exists &&
        collInfos[0].hasOwnProperty('options') &&
  ...
  dbs.databases.forEach(function(someDbInfo) {
    ...
    qeCollInfos.forEach(function(someCollInfo) {
      ...
      qeEntry['escCollectionInfo'] = (() => {
        const escColl = someDb.getCollection(
            someCollInfo.options.encryptedFields.escCollection);
        ...
        const anchorCount =
            escColl.countDocuments({'value': {'$exists': true}});
        const nonAnchorCount =
            escColl.countDocuments({'value': {'$exists': false}});
        return {
      ...
      qeEntry['ecocCollectionInfo'] = (() => {
        const ecocColl = someDb.getCollection(
            someCollInfo.options.encryptedFields.ecocCollection);
        const {exists, isClusteredCollection} =
        ...
        const compactTempCollectionExists =
            someDb.getCollectionInfos({name: ecocColl.getName() + '.compact'})
                .length > 0;
        return {
      ...
      qeEntry['safeContentInfo'] = (() => {
        const safeContentIndexed =
            (edcColl.getIndexes().find(
                 doc => doc.key.hasOwnProperty('__safeContent__')) !==
             undefined);
        const encryptedFieldPaths =
            someCollInfo.options.encryptedFields.fields
                .filter(field => field.hasOwnProperty('queries'))
                ...
                .aggregate([
                  {
                    $match: {
                      $and: [
                        {
                          $or: encryptedFieldPaths.map((field) => {
                            return {[field]: {$exists: true}};
                          })
                        },
                        {
                          $or: [
                            {__safeContent__: {$exists: false}},
                            {__safeContent__: {$size: 0}}
                          ]
                        }
                      ]
                    }
      ...
      if (isMongoS) {
        qeEntry['shardingInfo'] = (() => {
          ...
          let shardDoc =
              configDB.collections.findOne({_id: edcColl.getFullName()});
          if (!shardDoc) {
...
function updateDataInfoAsIncomplete(isMongoS) {
  for (i = 0; i < _output.length; i++) {
    if (_output[i].section != 'data_info') {
      continue;
    }
...
function printDataInfo(isMongoS) {
  section = 'data_info';
  var dbs = printInfo('List of databases', function() {
  ...
  if (dbs.databases) {
    dbs.databases.forEach(function(mydb) {
      var collections = printInfo(
          'List of collections for database \'' + mydb.name + '\'', function() {
            var collectionNames = []
      ...
      printInfo('Database stats (MB)', function() {
        return db.getSiblingDB(mydb.name).stats(1024 * 1024)
      }, section);
      if (!isMongoS) {
        printInfo(
            'Database profiler for database \'' + mydb.name + '\'', function() {
              return db.getSiblingDB(mydb.name).getProfilingStatus()
            }, section, false, {'db': mydb.name})
      }
      ...
      if (collections) {
        collections.forEach(function(col) {
          ...
          printInfo('Collection stats (MB)', function() {
            return collectionObject.stats(1024 * 1024)
          }, section);
          collections_counter++;
          if (collections_counter > _maxCollections) {
            var err_msg = 'Already asked for stats on ' + collections_counter +
                ' collections ' +
            ...
            if (_printJSON) {
              err_msg +=
                  'The "subsection" fields have been prefixed with "INCOMPLETE_" ' +
                  'to indicate that partial data has been outputted.'
            }

            throw {
              name: 'MaxCollectionsExceededException', message: err_msg
            }
          }
          if (mydb.name == 'config' || mydb.name == 'local' ||
              mydb.name == 'admin') {
            // The following command doesn't make any sense to config, local
            // and admin
            return
          }
          if (isMongoS) {
          ...
          printInfo('Index Stats', function() {
